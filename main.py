...
from forms import CreatePostForm, Registration_Form, Login_Form, CommentForm
...
def set_is_admin(user_x):
    """determines whether the current user is the admin"""
    result = db.engine.execute("SELECT MIN([id]) as [id] FROM users")
    for row in result:
        admin_id = row["id"]
        break
    return int(user_x.id) == int(admin_id)
...
def admin_only(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if set_is_admin(current_user.id):
            return abort(403)
...
def register():
    ...
    if form.validate_on_submit():
        if not login_exists(form.email.data):
            ...
            current_user.is_admin = set_is_admin(current_user)
...
def show_post(post_id):
    ...
    if form.validate_on_submit():
        if not current_user.is_authenticated:
            flash("You need to login/register before commenting.")
            return redirect(url_for("login"))
        new_comment = Comment(
